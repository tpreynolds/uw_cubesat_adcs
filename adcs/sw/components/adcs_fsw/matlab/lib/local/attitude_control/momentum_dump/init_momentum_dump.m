...
p_dump.ic.ang_mom_ref       = fsw_params.actuators.reaction_wheel.inertia_matrix*[min(fsw_params.bus.RW_RPM_thresh)*fsw_params.convert.RPM_2_RADPS;...
                                                     min(fsw_params.bus.RW_RPM_thresh)*fsw_params.convert.RPM_2_RADPS;...
                                                     min(fsw_params.bus.RW_RPM_thresh)*fsw_params.convert.RPM_2_RADPS]; %reference angular momentum of reaction wheels

% Sample time
p_dump.sample_time_s    = 1/5; % sample at 5 Hz

% Momentum unloading controller gains
p_dump.gain_k     = 1*10^(-7);
